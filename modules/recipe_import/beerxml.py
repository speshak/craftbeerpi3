...
class BeerXMLImport(FlaskView):
    ...
    def get(self):
        if not os.path.exists(self.BEER_XML_FILE):
            self.api.notify(headline="File Not Found", message="Please upload a BeerXML file.",
                            type="danger")
            return ('', 404)
        result = []
        ...
        try:
            if request.method == 'POST':
                ...
                if file and self.allowed_file(file.filename):
                    ...
                    self.api.notify(headline="Upload Successful", message="The BeerXML file was succesfully uploaded.")
                    return ('', 204)
                return ('', 404)
        except Exception as e:
            self.api.notify(headline="Upload Failed", message="Failed to upload BeerXML file.", type="danger")
            return ('', 500)
